...
Cinder.JquiSortableComponent = Ember.Component.extend(Cinder.BaseComponent, {
  ...
  sortableUpdate: function(event, sortable) {
    ...
    if (list) {
      ...
      emberComponent.dragInfo.set('item', draggedItem);
      emberComponent.dragInfo.set('oldIndex', emberComponent.originalContent.indexOf(draggedItem));
      emberComponent.dragInfo.set('newIndex', newContent.indexOf(draggedItem));
      emberComponent.sortables.set('content', newContent);
    }
  },
      ...
      self.sortables.reopen({
        move : function(from, to) {
          ...
          if ((from > -1) && (from < list.length) && (to > -1) && (to < list.length) && (to != from)) {
            ...
            self.dragInfo.set('item', movingItem);
            self.dragInfo.set('oldIndex', from);
            self.dragInfo.set('newIndex', to);
          }
        }
      });
      self.buildList(self);
...
Cinder.JquiTooltipComponent = Ember.Component.extend(Cinder.BaseComponent, {
  ...
  didInsertElement: function() {
    ...
    if (self.valueObject) {
      ...
      (self.$()).attr('title', self.valueObject.get("value"));
...
Cinder.JquiMenuComponent = Ember.Component.extend(Cinder.BaseComponent, {
    ...
    if (self.menu) {
      ...
      (self.$()).find(".cinder-menu").menu({
        select: function(event, ui) {
          ...
          self.menu.set('selection', selectedItem);
        }.bind(self)
      });
    }
  },
  didInsertElement: function() {
    ...
    self.addObserver(self, menuChanged, 'items', self.menu);
    var items = self.menu.content;
  },
...
Cinder.JquiSelectMenuComponent = Ember.Component.extend(Cinder.BaseComponent, {
  ...
  buildMenu: function(self) {
    if (self.menu) {
      var nameStr = self.menu.get('name') ? 'name="' + self.menu.get('name') + '" ' : '';
      var disabledStr = self.menu.get('disabled') ? ' disabled="disabled" ' : '';
  ...
  didInsertElement: function() {
    ...
    if (self.menu) {
      ...
      self.addObserver(self, menuChanged, 'items', self.menu);
      self.addObserver(self, menuChanged, 'disabled', self.menu);
...
Cinder.JquiSpinnerComponent = Ember.Component.extend(Cinder.BaseComponent, {
  ...
  spinnerStop: function(event, spinn) {
    ...
    this.valueObject.set('value', inputElement.spinner("value"));
    this.sendAction('stop');
  },

  buildSpinner: function(self) {
    var step = self.step ? self.step : 1;
    var min = self.min ? self.min : 1;
    var max = self.max ? self.max : 100;
    var w = self.width ? self.width : 175;
    var numberFormat = self.numberFormat ? self.numberFormat : undefined;
    self.spinner = (self.$()).find("input").spinner({
      step: step,
      min: min,
      max: max,
    ...
    if (self.valueObject) {
      (self.$()).find("input").spinner("value", self.valueObject.get('value'));
      var observingFn = function(observed) {
