  ...
  didInsertElement: function() {
    ...
    if (self.valueObject) {
      ...
      self.valueObject.addObserver('value', function() {
        console.log('date picker response');
        (self.$()).find("input").val(self.valueObject.get('value'));
      });
    }
  }
...
Cinder.TabsView = Ember.View.extend({
  ...
  didInsertElement: function() {
    ...
    self.contentSize = 0;
    if (self.content) {
      self.contentSize = self.content.value.length;
      var tabs_str = '<ul>';
      for (var i = 0; i < self.content.value.length; i++) {
        tabs_str += '<li><a href=\"#tabs-' + rand + '-' + i + '\">' + self.content.value[i][0] + '</a></li>';
      }
      tabs_str += '</ul>';

      for (var i = 0; i < self.content.value.length; i++) {
        tabs_str += '<div id=\"tabs-' + rand + '-' + i + '\">' + self.content.value[i][1] + '</div>';
      }

      ((self.$()).find('div')).append(tabs_str);
      self.content.addObserver('value', function() {
        console.log('content changed');
        ((self.$()).find('div div')).each(function(index) {
          $(this).text(self.content.value[index][1]);
        });
        ((self.$()).find('div ul li a')).each(function(index) {
          $(this).text(self.content.value[index][0]);
        });
      });
