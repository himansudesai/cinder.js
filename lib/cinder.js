...
Cinder.JquiMenuComponent = Ember.Component.extend(Cinder.BaseComponent, {
  ...
  buildMenu: function(self) {
    if (self.menu) {
      ...
      var items = self.menu.get('content');
  ...
  didInsertElement: function() {
    var self = this;
    if (self.menu.content) {
      self.buildMenu(self);
      var menuChanged = function() {
        self.buildMenu(self);
      }
      self.addObserver(self, menuChanged, 'content', self.menu);
  ...
  buildMenuStr: function(items) {
    ...
    for (var i=0; i<items.length; i++) {
      ...
      var enabled = items[i][item];
      if (Array.isArray(enabled)) {
        // we have sub-menu instead of boolean
        var subItems = enabled;
        str += "<li>" + item + "<ul>";
        str += self.buildMenuStr(subItems);
        str += "</li></ul>";
      } else {
        self.addObserver(self, itemChanged, item, items[i]);
        if (enabled) {
          str += '<li>' + item + '</li>';
        } else {
          str += '<li class="ui-state-disabled">' + item + '</li>';
        }
      }
    }
    return str;
